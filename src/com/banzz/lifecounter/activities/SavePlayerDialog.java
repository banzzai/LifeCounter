...
public class SavePlayerDialog extends DialogFragment {
	SavePlayerDialogListener mListener;
	private RadioGroup mRadio;
	private ListView mSelectUser;

	public void setListener(SavePlayerDialogListener mListener) {
		this.mListener = mListener;
	}

	public interface SavePlayerDialogListener {
			...
			if (mUsers != null && mUsers.length > 0) {
			    ...
			    mSelectUser.setOnItemClickListener(new OnItemClickListener() {
					...
					public void onItemClick(AdapterView<?> arg0, View arg1, int position, long arg3) {
						mListener.onValidateSave(mUsers[position], (mRadio.getCheckedRadioButtonId() == R.id.radio_player_1 ? Constants.PLAYER_ONE : Constants.PLAYER_TWO));
						dismiss();
					}
				});
			}
	    ...
	    builder.setView(view)
	           .setNegativeButton(R.string.cancel, new DialogInterface.OnClickListener() {
	               public void onClick(DialogInterface dialog, int id) {
	                   SavePlayerDialog.this.getDialog().cancel();
	               }
	           });      
